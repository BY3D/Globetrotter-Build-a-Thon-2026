...
// When --nodeps is passed, skip downloading all native dependencies (Foundry Local Core, ONNX Runtine, ONNX Runtime GenAI).
// Useful in CI when native binaries are already available on the agent or installed separately.
...
console.log(`[foundry-local] Skip FLC dependencies: ${noDeps}`);
...
let ARTIFACTS = [];
if (noDeps) {
    console.log(`[foundry-local] Skipping FLC download (--nodeps).`);
} else if (useWinML) {
